@testitem "FastMRI: parse email URLs" tags = [] begin
    using MRITestData
    # Unit-test the parser behind set_fastmri_urls! directly, so the test exercises the
    # shipped regex rather than a copy of it and stays free of Preferences side effects.
    ...
    urls, expires = MRITestData._parse_fastmri_email(sample_email)
    ...
    @test expires == 9999999999
end

@testitem "FastMRI: parse fails on empty input" tags = [] begin
    using MRITestData
    @test_throws ArgumentError MRITestData._parse_fastmri_email("no curl commands here")
    @test_throws ArgumentError MRITestData._parse_fastmri_email("")
end
